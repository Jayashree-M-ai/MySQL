...
where departments.department_name='Finance'; -- corrected
...
FROM sales
GROUP BY customer_id; -- corrected
...
SELECT product_name, price
FROM products
WHERE price > (select AVG(price) from products); -- corrected
...
SELECT DISTINCT customer_name
FROM orders
WHERE order_amount > 1000; -- corrected
